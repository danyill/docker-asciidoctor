...
. In the working path: `docker build --tag danyill/asciidoctor:latest .`
. We can build directly from the github repo with `docker build -t danyill/asciidoctor:latest git@github.com:danyill/docker-asciidoctor.git`
